...
import { splitBySaved, mergeAnalysis, normalizeTicker } from "./db.js";
...
describe("normalizeTicker", () => {
  ...
  it("accepts dotted and dashed class shares", () => {
  ...
  it("rejects garbage", () => {
    ...
    expect(normalizeTicker("   ")).toBe(null);
    expect(normalizeTicker("TOOLONGNAME")).toBe(null);
    expect(normalizeTicker("1; DROP TABLE")).toBe(null);
    expect(normalizeTicker(".AAPL")).toBe(null); // must start with a letter
    expect(normalizeTicker(null)).toBe(null);
    expect(normalizeTicker(42)).toBe(null);
  });
});
