...
/**
 * Newest stored brief, or null if there isn't a recent one.
 * `maxAgeHours` stops a stale brief from being presented as today's.
 */
export async function getLatestBrief(maxAgeHours = 24) {
  const sql = getClient();
  if (!sql) return null;
  try {
    await ensureBriefsTable(sql);
